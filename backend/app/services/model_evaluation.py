...
def train_and_evaluate(
    model_name, model, X_train, X_test, y_train, y_test, save_path=None
):
...
if __name__ == "__main__":  # pragma: no cover
    ...
    match_runs_df = match_data.merge(
    ...
    match_df = match_runs_df[
        match_runs_df["team1"].isin(teams) & match_runs_df["team2"].isin(teams)
    ].copy()
    match_df["team1"] = (
        match_df["team1"]
        .str.replace("Deccan Chargers", "Sunrisers Hyderabad")
        .str.replace("Delhi Daredevils", "Delhi Capitals")
    )
    match_df["team2"] = (
        match_df["team2"]
        .str.replace("Deccan Chargers", "Sunrisers Hyderabad")
    ...
    X_train, X_test, y_train, y_test = split_data(
        final_df.drop(columns=["match_id"]).copy()
    )
    ...
    train_and_evaluate(
        ...
        y_train,
        y_test,
        save_path="../ml_model/linear_regression_pipe.pkl",
    )
    train_and_evaluate(
        "Ridge Regression",
        Ridge(alpha=10),
        ...
        y_train,
        y_test,
        save_path="../ml_model/ridge_regression_pipe.pkl",
    )
    train_and_evaluate(
        "Lasso Regression",
        Lasso(alpha=0.001),
        ...
        y_train,
        y_test,
        save_path="../ml_model/lasso_regression_pipe.pkl",
    )
    train_and_evaluate(
        "KNN",
        ...
        y_train,
        y_test,
        save_path="../ml_model/knn_pipe.pkl",
    )
    train_and_evaluate(
        "Decision Tree",
        ...
        y_train,
        y_test,
        save_path="../ml_model/decision_tree_pipe.pkl",
    )
    train_and_evaluate(
        "Random Forest",
        RandomForestRegressor(
        ...
        y_train,
        y_test,
        save_path="../ml_model/random_forest_pipe.pkl",
    )
    train_and_evaluate(
        "Extra Trees",
        ExtraTreesRegressor(
        ...
        y_train,
        y_test,
        save_path="../ml_model/extra_trees_pipe.pkl",
    )
    train_and_evaluate(
        "AdaBoost",
        ...
        y_train,
        y_test,
        save_path="../ml_model/adaboost_pipe.pkl",
    )
    train_and_evaluate(
        "Gradient Boost",
        GradientBoostingRegressor(n_estimators=15, learning_rate=1.0),
        X_train,
        X_test,
        y_train,
        y_test,
        save_path="../ml_model/gradient_boost_pipe.pkl",
    )
    train_and_evaluate(
        "XGBoost",
        XGBRegressor(n_estimators=15, learning_rate=1.0),
        X_train,
        X_test,
        y_train,
        y_test,
        save_path="../ml_model/xgboost_pipe.pkl",
    )
    train_and_evaluate(
        "Logistic Regression",
        ...
        y_train,
        y_test,
        save_path="../ml_model/logistic_regression_pipe.pkl",
    )

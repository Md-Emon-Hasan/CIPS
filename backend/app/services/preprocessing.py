...
def calculate_inning_runs(delivery_df):
    ...
    try:
        ...
        total_score_df = (
            delivery_df.groupby(["match_id", "inning"])
            .sum()["total_runs"]
            .reset_index()
        )
...
def select_relevant_match_columns(match_df):
    ...
    try:
        ...
        logging.info(
            f"Shape after selecting relevant match columns: {relevant_df.shape}"
        )
        return relevant_df
    except Exception as e:
...
def filter_second_innings(merged_df):
    ...
    try:
        ...
        logging.info(
            f"Shape after filtering for second innings: {second_innings_df.shape}"
        )
...
def calculate_cumulative_runs(delivery_df):
    ...
    try:
        ...
        delivery_df["current_score"] = delivery_df.groupby("match_id")[
            "total_runs_y"
        ].cumsum()
...
def calculate_runs_left(delivery_df):
    ...
    try:
        ...
        delivery_df["runs_left"] = (
            delivery_df["total_runs_x"] - delivery_df["current_score"] + 1
        )
...
def calculate_wickets_left(delivery_df):
    ...
    try:
        ...
        wickets_fallen = (
            delivery_df.groupby("match_id")["player_dismissed"].cumsum().values
        )
...
def standardize_final_team_names(final_df):
    ...
    try:
        ...
        final_df["batting_team"] = final_df["batting_team"].str.replace(
            "Deccan Chargers", "Sunrisers Hyderabad"
        )
        final_df["bowling_team"] = final_df["bowling_team"].str.replace(
            "Deccan Chargers", "Sunrisers Hyderabad"
        )
        final_df["batting_team"] = final_df["batting_team"].str.replace(
            "Delhi Daredevils", "Delhi Capitals"
...
if __name__ == "__main__":  # pragma: no cover
    ...
    match_runs_df = merge_total_runs_with_match(
        match_data.copy(), total_score_df.copy()
    )
    ...
    final_df_handled_city = handle_missing_city(
        final_df_standardized_teams.copy(), cities_dict
    )

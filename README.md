...
Predicting who's likely to win a cricket match mid-game usually comes down to a commentator's gut feeling. CIPS replaces that guesswork with **a live win probability**: enter the teams, ground, score, wickets, and overs, and it returns each side's chance of winning in real time. Every prediction is **timestamped and saved** for later checking. Built and tested like **real production software**, not a demo.

Under the hood, a FastAPI backend serves an **XGBoost** classifier inside a **scikit-learn Pipeline**, so **OneHotEncoding** and the feature set (runs left, balls left, wickets left, run rates) apply identically at training and inference. Trained on IPL ball-by-ball history (2008–2019) with franchise standardization and Duckworth-Lewis filtering. Every request is persisted via **SQLModel** into SQLite with a timestamped audit trail. A **React/Vite/Tailwind** frontend calls the API, backed by **Ruff**, **Mypy**, **Pytest**, and a **Docker**-based GitHub Actions pipeline — reproducible, tested, containerized: the discipline a company needs before trusting a model in production.
